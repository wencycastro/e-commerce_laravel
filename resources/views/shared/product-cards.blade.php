        ...
        @foreach ($products as $product)
            <div class="col">
                <div class="card h-100 shadow-sm border-0 card-custom">
                    <img src="{{ $product->image }}" class="card-img-top card-img-custom" alt="Product Image">
                    <div class="card-body">
                        <h5 class="card-title">{{ $product->name }}</h5>
                        <p class="card-text text-light">{{ $product->description }}</p>
                    </div>
                    <div class="card-footer bg-transparent">
                        <a href="{{ route('edit_product', $product->id) }}" class="btn btn-secondary w-100 mt-2">Edit</a>
